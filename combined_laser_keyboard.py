...
try:
    ...
    print("keyboard not installed. Please run: pip install keyboard")
    sys.exit(1)
...
# --- LIVE PLOT CLASS ---
class LivePlot:
    def __init__(self, acquisition_thread, robot):
        ...
        try:
            keyboard.add_hotkey('n', self.decrease_speed)
            keyboard.add_hotkey('m', self.increase_speed)
            keyboard.add_hotkey('space', self.emergency_stop)
            keyboard.add_hotkey('esc', self.stop_program)
            keyboard.add_hotkey('left', self.rotate_left)
            keyboard.add_hotkey('right', self.rotate_right)
        except Exception as e:
            print(f"Warning: Could not register hotkeys: {e}")

    def decrease_speed(self):
        self.current_speed = max(0.0, self.current_speed - 10.0)
        print(f"Speed limit: {self.current_speed}")

    def increase_speed(self):
        self.current_speed = self.current_speed + 10.0
        print(f"Speed limit: {self.current_speed}")
    ...
    def rotate_left(self):
        self.thread.rotation_angle -= 0.01
        print(f"Rotation angle: {self.thread.rotation_angle:.2f}°")
    ...
    def update(self, _):
        # --- 1. Handle Keyboard Input (Continuous) ---
        # W/S -> A2 (Forward/Back)
        # A/D -> A1 (Left/Right)
        # Q/E -> A3 (Up/Down)
        
        val_a1 = 0.0
        if keyboard.is_pressed('d'): val_a1 += 1.0
        if keyboard.is_pressed('a'): val_a1 -= 1.0
        
        val_a2 = 0.0
        if keyboard.is_pressed('w'): val_a2 += 1.0
        if keyboard.is_pressed('s'): val_a2 -= 1.0
        
        val_a3 = 0.0
        if keyboard.is_pressed('q'): val_a3 += 1.0
        if keyboard.is_pressed('e'): val_a3 -= 1.0
        
        speeds["A1"] = val_a1 * self.current_speed
        speeds["A2"] = val_a2 * self.current_speed
        speeds["A3"] = val_a3 * self.current_speed
    ...
    try:
        ...
        try:
            ...
            with safe_transfer(hLLT):
                ...
                print("Starting main loop. Close window or press Esc to exit.")
                plt.show()
